...
type Reservation struct {
	...
	SuggestedTimeslots pq.StringArray    `json:"suggested_timeslots" gorm:"type:text[]"`
}
...
type CreateReservationRequest struct {
	...
	SuggestedTimeslots []string `json:"suggestedTimeslots" binding:"required,min=1"`
}

type UpdateReservationStatusRequest struct {
	Status     ReservationStatus `json:"status" binding:"required"`
	PickupSlot string            `json:"pickupSlot,omitempty"`
}
